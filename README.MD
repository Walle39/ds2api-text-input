...
- `runtime`：账号并发、队列与 token 刷新策略，可通过 Admin Settings 热更新。包含 `request_interval_ms`（请求间隔毫秒数，默认 500ms，防止频繁请求被封禁）和 `account_sticky_enabled`（账号粘性，默认开启，优先使用上次使用的账号，避免连续对话中切换账号被判定为异常）。
